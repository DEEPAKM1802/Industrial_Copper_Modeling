...
def app():
    ...
    if file is not None:
        ...
        with tab1:
            task = 'Regression'
            models_reg = ['All', 'Linear Regression', 'Lasso Regression', 'Ridge Regression', 'ElasticNet',
                      'Decision Tree Regressor', 'Extra Trees Regressor', 'XGB Regressor']
            model_reg = st.selectbox('Select model', models_reg)
            ...
            elif model_reg == 'Lasso Regression':
                results = ml_execution(df, task, 'Lasso Regression')
            elif model_reg == 'Ridge Regression':
                results = ml_execution(df, task, 'Ridge Regression')
            elif model_reg == 'ElasticNet':
                results = ml_execution(df, task, 'ElasticNet')
            elif model_reg == 'Decision Tree Regressor':
                results = ml_execution(df, task, 'Decision Tree Regressor')
            elif model_reg == 'Extra Trees Regressor':
                results = ml_execution(df, task, 'Extra Trees Regressor')
            elif model_reg == 'XGB Regressor':
                results = ml_execution(df, task, 'XGB Regressor')

        with tab2:
            task2 = 'Classification'
            models_cls = ['All', 'Logistic Regression', 'KNN', 'Decision Tree Classifier', 'Voting Classifier',
                      'Random Forest Classifier', 'XGB Classifier']

            model_cls = st.selectbox('Select model', models_cls)

            # Run the ML code on the DataFrame with the selected model
            if model_cls == 'Logistic Regression':
                results = ml_execution(df, task2, 'Logistic Regression')
            elif model_cls == 'KNN':
                results = ml_execution(df, task2, 'KNN')
            elif model_cls == 'Decision Tree Classifier':
                results = ml_execution(df, task2, 'Decision Tree Classifier')
            elif model_cls == 'Voting Classifier':
                results = ml_execution(df, task2, 'Voting Classifier')
            elif model_cls == 'Random Forest Classifier':
                results = ml_execution(df, task2, 'Random Forest Classifier')
            elif model_cls == 'XGB Classifier':
                results = ml_execution(df, task2, 'XGB Classifier')
